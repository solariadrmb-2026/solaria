...
import { onAuthStateChanged, singOut }
from 
...
onAuthStateChanged( auth, user => { 
  if (!user) { localStorage.removeItem("logado");
 ...
 function iniciarTempoReal() { 
    ...
    onValue(ref(db, "solaria/sensores/temperatura"),
snap =>
...
document.querySelector(".desligar");
  ...
  window.setBomba = function (estado) {
  ...
  function atualizar(id, valor, sufixo = "") {
    ...
    if (id === "luz") {
        ...
        if (valor >= 0 && valor <10) classificação = "- Muito Escuro ";
        else if (valor >= 10 && valor < 50) classificação = "- Escuro";
        else  if (valor >= 50 && valor < 300) classificação = "- Luz Fraca";
        else if (valor >= 300 && valor < 500) classificação = "- Luz Moderada";
        else if (valor >= 500 && valor < 1000) classificação = "- Luz Adequada";
        else if (valor >= 1000 && valor < 2000) classificação = "- Luz Brilhante";
        else if (valor >=2000) classificação = "- Luz Intensa";
